...
void HTTP_IO_Data::setState(commandstatus iState)
{
    ...
    if (commandstatus::Success == iState || commandstatus::Fail == iState)
    {
        m_cv.notify_one();
    }
}
...
void HTTP_IO_Data::wait()
{
    std::unique_lock<std::mutex> lock(m_CommandMutex);
    m_cv.wait(lock, [this] {
        return m_iState == commandstatus::Success || m_iState == commandstatus::Fail;
    });
}
